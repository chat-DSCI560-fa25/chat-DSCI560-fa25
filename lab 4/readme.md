...
Open the new .env file and fill in your MySQL database credentials.

The new .env file with a text editor. Replace the placeholder values (like changeme, your_user, your_password, etc.) with your desired MySQL database credentials. The MYSQL_HOST should be the name of your database service in docker-compose.yml
...
The consensus.py module contains a trend-following, multi-indicator strategy. It presumes that a high-probability trade signal is one that is confirmed by multiple, distinct types of indicators simultaneously. The MACD crossover provides a measure of medium-term momentum, the RSI > 50 condition confirms short-term buying pressure, and the close > sma_20 confirms the immediate price trend. A buy signal is only generated when all three agree, which is intended to filter out weak or ambiguous signals.



Indicator Calculation

## The function first calculates a set of technical indicators on the input DataFrame df using the pandas-ta library:

df.ta.macd(fast=12, slow=26, signal=9): Generates MACD (MACD_12_26_9) and Signal (MACDs_12_26_9) lines.

df.ta.rsi(length=14): Generates the Relative Strength Index (RSI_14).

df.ta.sma(length=20): Generates a short-term Simple Moving Average (SMA_20).

df.ta.sma(length=200): Generates a long-term Simple Moving Average (SMA_200) to act as a trend filter.

After calculation, a df.rename operation standardizes the column names to macd_line, signal_line, rsi_14, sma_20, and sma_200.

Scoring Logic

## A new integer column named score is instantiated and initialized to 0. The score is then modified based on six boolean conditions:

score is incremented by 1 if df['macd_line'] > df['signal_line'].

score is incremented by 1 if df['rsi_14'] > 50.

score is incremented by 1 if df['close'] > df['sma_20'].

score is decremented by 1 if df['macd_line'] < df['signal_line'].

score is decremented by 1 if df['rsi_14'] < 50.

score is decremented by 1 if df['close'] < df['sma_20'].

Signal Generation

## A final signal column is initialized to 0. The buy and sell signals are determined by two variables:

buy_condition: This variable evaluates to True if (df['score'] == 3) & (df['close'] > df['sma_200']).

sell_condition: This variable evaluates to True if df['score'] == -3.

Using df.loc, the signal column is set to 1 where buy_condition is true, and -1 where sell_condition is true.

2. EMA + RSI Crossover Algorithm

The ema_rsi.py module contains a mean-reversion and momentum strategy defined within the generate_signals(df, params) function.

The rsi_14 < 45 condition is designed to find stocks that have recently experienced selling pressure or are in an "oversold" state, representing a potential entry at a relative price low. However, to avoid entering a falling market, the strategy does not act on the RSI condition alone. It requires a confirmation signal from the ema_10 > ema_30 crossover. This crossover indicates that short-term momentum is turning positive and buyers are beginning to take control.

Indicator Calculation

## The function calculates three indicators on the input df:

df.ta.ema(length=10): Generates a fast Exponential Moving Average (EMA_10).

df.ta.ema(length=30): Generates a slow Exponential Moving Average (EMA_30).

df.ta.rsi(length=14): Generates the Relative Strength Index (RSI_14).

A df.rename operation follows, standardizing these column names to ema_10, ema_30, and rsi_14.

Signal Generation

## A signal column is initialized to 0. The buy and sell signals are determined by two variables:

buy_condition: This variable evaluates to True if (df['rsi_14'] < 45) & (df['ema_10'] > df['ema_30']). This checks for an oversold condition combined with a bullish momentum crossover.

sell_condition: This variable evaluates to True if df['rsi_14'] > 65. This checks for an overbought condition.

Using df.loc, the signal column is set to 1 where buy_condition is true, and -1 where sell_condition is true.

# Model 2: Optimized SMA + RSI Trading Strategy
...
# Model 3: Stock Portfolio Trading System
